...
int main(int argc, char** argv) {
    ...
    switch (argc) {
        ...
        case (3):
            ...
            if (snake_grows != 1 && snake_grows != 0) {
                ...
                printf("Your input was incorrect please fix.\n");
                return 0;
            }
            break; 
        case(4):
            ...
            status = initialize_game(&cells, &width, &height, &snake, argv[3]);
            break;
        case (1):
        default:
            printf("usage: snake <GROWS: 0|1> [EXTRA CREDIT: 0|1] [BOARD STRING]\n");
            return 0;
    }
    ...
    if(status == INIT_ERR_INCORRECT_DIMENSIONS){
        printf("The wrong dimensions were entered.");
        return EXIT_FAILURE;
    } else if (status == INIT_ERR_WRONG_SNAKE_NUM){ 
        printf("No snake or multiple snakes are on the board");
        return EXIT_FAILURE;
    } else if (status == INIT_ERR_BAD_CHAR) {
        printf("any other part of the compressed string was formatted incorrectly");
        return EXIT_FAILURE;
    }
    ...
    // memcpy(g_name, name_buffer, 1000);
